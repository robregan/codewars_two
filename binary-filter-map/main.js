...
function bitsBattle(number){
    const odds = (number.filter(e => e % 2!=0).map(e=>e.toString(2)).join('').match(/1/g) || []).length
   const evens = (number.filter(e => e % 2===0).map(e=>e.toString(2)).join('').match(/0/g) || []).length
